...
export default function FlashcardsViewer({ flashcards, userName, initialIndex, onIndexChange, onStatsChange }: FlashcardsViewerProps) {
  ...
  function handleNext() {
    setFlipped(false)
    setTimeout(() => goToIndex(currentIndex + 1), 150)
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '10px', marginBottom: '14px' }}>
        ...
        {user && (
          <button onClick={() => toggleImportant(current.id)} title={isCurrentImportant ? 'Remove from important' : 'Mark as important'}
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '7px', marginBottom: '20px' }}>
        {cards.map((_, i) => (
          <span key={i} onClick={() => { goToIndex(i); setFlipped(false) }}
            style={{ width: i === currentIndex ? '22px' : '7px', height: '7px', borderRadius: '99px', background: i === currentIndex ? '#2563EB' : '#D6DCE7', transition: 'width 0.3s ease, background 0.3s ease', cursor: 'pointer', display: 'inline-block' }}
          />
        ))}
      </div>
      ...
      <div style={{ position: 'relative', marginBottom: '16px' }}>
        ...
        <div style={{ position: 'relative', perspective: '2000px', width: '100%', height: 'clamp(270px, 42vw, 360px)' }}>
          <div onClick={() => setFlipped(!flipped)}
            style={{ position: 'relative', width: '100%', height: '100%', transformStyle: 'preserve-3d', transition: 'transform 0.55s cubic-bezier(0.34, 1.1, 0.64, 1)', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)', cursor: 'pointer' }}>

            {/* FRONT */}
            <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', background: 'linear-gradient(#ffffff 0%, #FCFDFF 100%)', border: '1px solid #ECEEF3', borderRadius: '24px', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.6), 0 1px 2px rgba(16,24,40,0.04), 0 10px 20px -10px rgba(16,24,40,0.12), 0 28px 50px -28px rgba(37,99,235,0.20)', display: 'flex', flexDirection: 'column', padding: '30px 32px', textAlign: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                <span style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.08em', color: '#2563EB', textTransform: 'uppercase', background: '#EEF3FF', padding: '5px 12px', borderRadius: '99px' }}>Term</span>
                <span style={{ fontSize: '12px', fontWeight: 700, color: '#94A3B8' }}>Card {currentIndex + 1} / {total}</span>
              </div>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', minHeight: 0 }}>
                <div style={{ width: '100%' }}>{renderContent(current.front_text, 'front')}</div>
              </div>
            ...
            {/* BACK */}
            <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)', background: 'linear-gradient(#F3F7FF 0%, #EAF1FF 100%)', border: `1px solid ${isCurrentImportant ? '#FCD34D' : '#DBE9FE'}`, borderRadius: '24px', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.5), 0 1px 2px rgba(16,24,40,0.04), 0 10px 20px -10px rgba(16,24,40,0.12), 0 28px 50px -28px rgba(37,107,255,0.25)', display: 'flex', flexDirection: 'column', padding: '30px 32px', textAlign: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                <span style={{ fontSize: '11px', fontWeight: 800, letterSpacing: '0.08em', color: '#2563EB', textTransform: 'uppercase' }}>Definition</span>
                <span style={{ fontSize: '12px', fontWeight: 700, color: '#6C86C4' }}>Card {currentIndex + 1} / {total}</span>
              </div>
      ...
      {!flipped ? (
        <div style={{ display: 'flex', gap: '14px', marginTop: '24px' }}>
          <button onClick={handlePrev} disabled={currentIndex === 0}
            style={{ flex: 1, height: '52px', borderRadius: '14px', border: '1px solid #E2E8F0', background: '#fff', color: '#475569', fontSize: '14px', fontWeight: 700, cursor: currentIndex === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', opacity: currentIndex === 0 ? 0.45 : 1, transition: 'transform 0.15s' }}>
          ...
          <button onClick={() => setFlipped(true)}
            style={{ flex: 2, height: '52px', borderRadius: '14px', border: 'none', background: '#2563EB', color: '#fff', fontSize: '14.5px', fontWeight: 700, cursor: 'pointer', transition: 'transform 0.15s' }}>
            Show Answer
          </button>
          <button onClick={handleNext} disabled={currentIndex === total - 1}
            style={{ flex: 1, height: '52px', borderRadius: '14px', border: '1px solid #E2E8F0', background: '#fff', color: '#475569', fontSize: '14px', fontWeight: 700, cursor: currentIndex === total - 1 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', opacity: currentIndex === total - 1 ? 0.45 : 1, transition: 'transform 0.15s' }}>
            Next
        ...
        <div style={{ display: 'flex', gap: '14px', marginTop: '24px' }}>
          <button onClick={handleNeedsReview}
            style={{ flex: 1, height: '52px', borderRadius: '14px', border: '1px solid #FCD34D', background: '#FFFBEB', color: '#D97706', fontSize: '14px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'transform 0.15s' }}>
          ...
          <button onClick={handleGotIt}
            style={{ flex: 2, height: '52px', borderRadius: '14px', border: 'none', background: '#16A34A', color: '#fff', fontSize: '14.5px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'transform 0.15s' }}>
