...
// Find the line with mn-mobile-features
let startLine = -1
let endLine = -1

for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('mn-mobile-features')) startLine = i
  if (startLine !== -1 && lines[i].includes('Mockup') && i > startLine) {
    endLine = i - 1
    break
  }
}

console.log(`Found block: lines ${startLine + 1} to ${endLine + 1}`)
...
const newBlock = `      {/* Mobile-only feature highlights */}
      <div className="mn-mobile-features" style={{ display: 'none', flexDirection: 'column' as const, gap: '12px', marginTop: '24px' }}>
        {([
          {
            title: 'Lectures & Sheets',
            desc: 'Organized content for',
            descBold: 'every subject',
            bg: '#EEF2FF',
            color: '#4F46E5',
            icon: (
          ...
          },
          {
            ...
            desc: 'Study smarter with',
            descBold: 'active recall',
            bg: '#FDF4FF',
            color: '#9333EA',
            icon: (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {
            title: 'Previous Years Bank',
            desc: 'Past papers sorted by',
            descBold: 'year and type',
            bg: '#FFF7ED',
            color: '#EA580C',
            icon: (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          },
          {
            ...
            desc: 'Track your learning with',
            descBold: 'star ratings',
            bg: '#ECFDF5',
            color: '#16A34A',
            icon: (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div key={f.title} style={{ display: 'flex', alignItems: 'center', gap: '14px', background: '#fff', border: '1px solid #E8ECF2', borderRadius: '16px', padding: '14px 16px', boxShadow: '0 1px 3px rgba(15,23,42,0.06)' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '12.5px', color: '#64748B', marginTop: '2px' }}>
                {f.desc} <span style={{ color: f.color, fontWeight: 600 }}>{f.descBold}</span>
              </div>
            </div>
          </div>
        ))}
