...
import FlashcardsBankClient from '@/components/student/FlashcardsBankClient'

interface PageProps {
  params: Promise<{ uniSlug: string; subjectSlug: string }>
}

export default async function FlashcardsBankPage({ params }: PageProps) {
  ...
  // Get all flashcards across all lectures
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-400 mb-6 flex-wrap">
        ...
        <span>/</span>
        <span className="text-slate-700 dark:text-slate-200 font-medium">Flashcards Bank</span>
      </nav>

      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-slate-800 dark:text-slate-100">Flashcards Bank</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          {subRow.name} — all flashcards in one place
        </p>
      </div>
      ...
      {hasAccess ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-10 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Subscribe to {subRow.name} to access the Flashcards Bank.
          </p>
