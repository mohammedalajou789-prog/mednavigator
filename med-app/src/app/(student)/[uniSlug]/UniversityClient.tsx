...
// Category label shown inside the green badge on each card
function getCategoryLabel(category: string | null): string {
  if (category === 'preclinical')    return 'Pre-Clinical'
  if (category === 'clinical_major') return 'Clinical'
  if (category === 'clinical_minor') return 'Minor'
  return 'General'
}

// Access badge: star icon + label (always amber for Free, red for Premium, mixed for Mixed)
function getAccessStyle(access_mode: string): { color: string; bg: string; label: string } {
  if (access_mode === 'free')    return { color: '#D97706', bg: 'rgba(217,119,6,0.11)',  label: 'Free'    }
  if (access_mode === 'mixed')   return { color: '#D97706', bg: 'rgba(217,119,6,0.11)',  label: 'Mixed'   }
  if (access_mode === 'premium') return { color: '#DC2626', bg: 'rgba(220,38,38,0.11)',  label: 'Premium' }
...
export default function UniversityClient({
  ...
  const tabs = [
    { id: 'all', label: 'All' },
    ...sections.map(s => ({ id: s.key, label: s.tabLabel })),
  ]
  ...
  return (
    <>
      <style>{`
        ...
        .mn-subj-card { transition:transform .25s ease,box-shadow .25s ease; }
        .mn-subj-card:hover { transform:translateY(-4px); box-shadow:0 16px 32px -14px rgba(37,99,235,.28) !important; }
        .mn-save-btn  { transition:background .2s ease,color .2s ease,transform .15s ease; }
        .mn-save-btn:hover { transform:scale(1.05); }
      `}</style>
        ...
        <main style={{ padding:'clamp(16px,4vw,28px) clamp(16px,4vw,28px) 64px', maxWidth:1200, margin:'0 auto' }}>
          ...
          <div style={{ position:'relative', borderRadius:22, overflow:'hidden', padding:'28px clamp(20px,4vw,32px)', marginBottom:32, background:'linear-gradient(120deg,#EFF4FF,#F5F1FF 60%,#EEFCF3)', animation:'fadeUp .5s ease .05s backwards' }}>
            ...
            <div style={{ position:'relative', display:'flex', alignItems:'center', gap:20, flexWrap:'wrap' }}>
              ...
              {/* Logo 128×128 */}
              <div style={{ width:128, height:128, borderRadius:'50%', border:'1px solid #E2E8F0', overflow:'hidden', flexShrink:0, background:'#fff', boxShadow:'0 10px 26px -8px rgba(15,23,42,0.2)', animation:'logoPop .6s cubic-bezier(.34,1.56,.64,1) .15s backwards', position:'relative' }}>
                {university.logo_url ? (
                  <>
                  ...
                  <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', background:'#2563EB', color:'#fff', fontSize:36, fontWeight:800 }}>
              ...
              <div style={{ flex:1, minWidth:240, position:'relative' }}>
                ...
                {university.description && (
                  <p style={{ fontSize:13.5, lineHeight:1.6, color:'#94A3B8', marginTop:12, maxWidth:500 }}>
                    {university.description}
                  </p>
                )}

                {/* Country badge — absolute positioned like the reference */}
                {university.country && (
                  <div style={{ position:'absolute', top:30, right:0, zIndex:1, display:'flex', alignItems:'center', gap:8, padding:'8px 14px', borderRadius:12, background:'#fff', border:'1px solid rgba(37,99,235,0.15)', boxShadow:'0 4px 12px -4px rgba(15,23,42,.1)', flexShrink:0 }}>
                ...
                <div style={{ position:'absolute', top:-71, right:-10, width:220, height:220, borderRadius:'50%', background:'rgba(37,99,235,0.08)', pointerEvents:'none' }} />
              </div>
              ...
              <div style={{ display:'inline-flex', gap:3, background:'#EEF2F7', borderRadius:12, padding:4 }}>
                {tabs.map(tab => (
                  <button
                    ...
                    style={{
                      fontSize:13, fontWeight:600, padding:'7px 15px', borderRadius:9, cursor:'pointer', border:'none',
                      transition:'background .2s ease,color .2s ease,box-shadow .2s ease',
                      background: activeCategory === tab.id ? '#fff' : 'transparent',
                      color:      activeCategory === tab.id ? '#0F172A' : '#64748B',
                      boxShadow:  activeCategory === tab.id ? '0 1px 3px rgba(15,23,42,.12)' : 'none',
                    }}
                  >
                    {tab.label}
                  </button>
                ...
                <div key={section.key}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(min(100%,340px),1fr))', gap:16 }}>
                    {section.list.map((s, idx) => {
                      ...
                      const accessSt  = getAccessStyle(s.access_mode)
                      const isSaved   = saved.has(s.id)
                      const delay     = `${0.05 * (idx % 6)}s`

                      return (
                        <Link key={s.id} href={`/${uniSlug}/${s.slug ?? s.id}`} prefetch={false} style={{ textDecoration:'none', display:'block' }}>
                          <div
                            className="mn-subj-card"
                            style={{
                              position:'relative', background:'linear-gradient(135deg,#EFF4FF,#F5F1FF)',
                              ...
                              animation:'fadeUp .5s ease backwards', animationDelay:delay,
                            }}
                          >
                            <div style={{ padding:20, display:'flex', flexDirection:'column', flex:1 }}>

                              {/* Top row: category badge + access badge + save button */}
                              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:10 }}>
                                <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
                                  {/* Category badge — green */}
                                  <span style={{ fontSize:11, fontWeight:700, padding:'4px 9px', borderRadius:7, background:'rgba(22,163,74,0.11)', color:'#16A34A' }}>
                                    {catLabel}
                                  </span>
                                  {/* Access badge — amber star + label */}
                                  <span style={{ fontSize:11, fontWeight:700, padding:'4px 9px', borderRadius:7, background:accessSt.bg, color:accessSt.color, display:'inline-flex', alignItems:'center', gap:4 }}>
                                ...
                                {/* Save button — bookmark icon + text */}
                                {userId && (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      borderRadius:99, padding:'6px 11px', fontSize:11.5, fontWeight:700,
                                      cursor:'pointer',
                                      background: isSaved ? 'rgba(37,99,235,0.12)' : '#fff',
                                      color:      isSaved ? '#2563EB' : '#64748B',
                                      border:     `1px solid ${isSaved ? 'rgba(37,99,235,0.3)' : '#E2E8F0'}`,
                                    }}
                                  >
                              ...
                              <div style={{ display:'flex', gap:20, marginTop:16 }}>
                                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                                  ...
                                  <div>
                                    <div style={{ fontSize:13, fontWeight:700, color:'#0F172A' }}>{s.lecture_count}</div>
                                ...
                                <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                                  ...
                                  <div>
                                    <div style={{ fontSize:13, fontWeight:700, color:'#0F172A' }}>{s.chapter_count}</div>
                              ...
                              <div style={{ marginTop:16, paddingTop:14, borderTop:'1px solid rgba(37,99,235,0.12)' }}>
                                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
                                  ...
                                  <span style={{ fontSize:13, fontWeight:800, color:'#2563EB' }}>0%</span>
                                </div>
                                <div style={{ height:7, borderRadius:99, background:'#E2E8F0', overflow:'hidden' }}>
                                  <div style={{ height:'100%', width:'0%', borderRadius:99, background:'linear-gradient(90deg,#2563EB,#7C3AED)', transition:'width 1.1s cubic-bezier(.22,1,.36,1)' }} />
                                </div>
                                <div style={{ fontSize:11.5, color:'#94A3B8', marginTop:6 }}>0 of {s.lecture_count} lectures mastered</div>
                              </div>
