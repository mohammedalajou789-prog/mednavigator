...
export default function SheetPage() {
  ...
  // ── Single query: sheet content + saved position (all in one round trip) ──
  const { data: sheetData, isLoading: sheetLoading } = useQuery({
    queryKey: ['sheet-full', meta?.lecture?.id, meta?.userId],
    queryFn: async () => {
      const lectureId = meta!.lecture!.id
      const userId    = meta!.userId

      // Fetch sheet + progress in parallel
      const [sheetResult, progressResult] = await Promise.all([
        ...
        userId
          ? supabase.from('user_progress')
              ...
              .eq('user_id', userId).eq('lecture_id', lectureId).eq('content_type', 'sheet')
              .maybeSingle()
          : Promise.resolve({ data: null }),
      ])

      // Fetch image slots after we have sheet ID
  ...
  // ── Send progress + TOC to sidebar once data loads ────────────────────────
  useEffect(() => {
    if (!sheetData) return
  ...
  // Strategy: poll until scrollHeight is large enough, then scroll
  useEffect(() => {
  ...
  const handleProgressUpdate = useCallback((pct: number) => {
    ...
    // Always save scroll to localStorage immediately (no debounce)
