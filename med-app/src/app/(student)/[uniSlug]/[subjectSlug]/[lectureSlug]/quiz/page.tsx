...
function getLocalKey(lectureId: string) {
  return `lecture:${lectureId}:quiz_index`
}
...
export default function QuizPage() {
  ...
  useEffect(() => {
    ...
    ;(supabase as any)
      .from('lecture_resume_state').select('quiz_index')
      .eq('user_id', user.id).eq('lecture_id', meta.lecture.id).maybeSingle()
      .then(({ data }: { data: { quiz_index: number } | null }) => {
        const idx = data?.quiz_index ?? 0
  ...
  const saveIndex = useCallback((index: number) => {
    ...
    dbSaveTimer.current = setTimeout(async () => {
      await (supabase as any).rpc('save_resume_state', {
        ...
        p_active_tab: 'quiz', p_sheet_scroll: null, p_summary_scroll: null,
        p_flashcard_index: null, p_quiz_index: index, p_pyq_index: null,
      })
    }, 2000)
  ...
  useEffect(() => {
    ...
    function handleUnload() {
      ...
      navigator.sendBeacon('/api/save-resume', JSON.stringify({
        p_user_id: user!.id, p_lecture_id: meta!.lecture!.id,
        p_active_tab: 'quiz', p_quiz_index: currentIndexRef.current,
      }))
    }
  ...
  const handleStatsChange = useCallback((stats: { total: number; answered: number; correct: number; current: number; important: number }) => {
  ...
  return (
    <>
      ...
      <div style={{ padding: '0 clamp(12px, 3vw, 26px) 24px' }}>
        ...
        ) : quizLoading || resolvedIndex === null ? (
          <ContentSkeleton />
        ) : questions.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: '#94A3B8' }}>
            <p style={{ fontSize: '15px', fontWeight: 500 }}>No quiz questions available for this lecture.</p>
          </div>
        ) : (
          <QuizViewer
            key={`quiz-${resolvedIndex}`}
