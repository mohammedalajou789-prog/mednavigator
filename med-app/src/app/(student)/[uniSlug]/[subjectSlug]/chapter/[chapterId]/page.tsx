...
export default async function ChapterPage({ params }: PageProps) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--ink)', fontFamily: '"Plus Jakarta Sans", system-ui, -apple-system, sans-serif' }}>
      <main style={{ maxWidth: 860, margin: '0 auto', padding: 'clamp(16px, 4vw, 28px) clamp(16px, 4vw, 28px) 64px' }}>
        ...
        <div style={{ borderRadius: 20, padding: 'clamp(18px,4vw,28px) clamp(18px,4vw,30px)', marginBottom: 28, background: 'linear-gradient(120deg,rgb(232,240,255) 0%,rgb(245,248,255) 100%)', border: '1px solid rgb(223,232,251)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', color: 'rgba(47,107,255,0.7)', marginBottom: 8 }}>{groupLabel.toUpperCase()}</div>
          ...
          <div style={{ position: 'relative', width: 80, height: 80, flexShrink: 0 }}>
            <svg width="80" height="80" viewBox="0 0 80 80" style={{ transform: 'rotate(-90deg)' }}>
              ...
              <circle cx="40" cy="40" r="33" fill="none" stroke="#2F6BFF" strokeWidth="7" strokeLinecap="round"
                strokeDasharray="207.3" strokeDashoffset={207.3 * (1 - progressPercent / 100)}/>
            </svg>
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: 18, fontWeight: 800, color: '#2F6BFF', letterSpacing: '-0.03em' }}>{progressPercent}%</span>
            </div>
          </div>
        </div>
