...
export default function SummaryPage() {
  ...
  const { data: summaryData, isLoading: summaryLoading } = useQuery({
    queryKey: ['summary-content', meta?.lecture?.id],
    queryFn: async () => {
      const lectureId = meta!.lecture!.id
      const summaryResult = await supabase.from('summaries').select('id, content, status').eq('lecture_id', lectureId).maybeSingle()
      const summaryId = summaryResult.data?.id ?? ''
      const slotsResult = summaryId
        ? await supabase.from('image_slots').select('slot_number, media_library(file_url)').eq('entity_id', summaryId).eq('entity_type', 'summary')
        : { data: [] }
      ...
      return { summary: summaryResult.data, imageSlots }
    },
  ...
  const { data: resumeState } = useQuery({
    queryKey: ['resume-state-summary', user?.id, meta?.lecture?.id],
    queryFn: async () => {
      const { data } = await (supabase as any)
        .from('lecture_resume_state')
        .select('summary_scroll')
        .eq('user_id', user!.id)
        .eq('lecture_id', meta!.lecture!.id)
        .maybeSingle()
      return data as { summary_scroll: number } | null
    },
  ...
  const { data: savedProgress } = useQuery({
    queryKey: ['progress', user?.id, meta?.lecture?.id, 'summary'],
    queryFn: async () => {
  ...
  useEffect(() => {
    ...
    const localScroll = meta?.lecture?.id ? parseInt(localStorage.getItem(`lecture:${meta.lecture.id}:summary_scroll`) ?? '0', 10) : 0
    const targetScroll = localScroll > 0 ? localScroll : (resumeState?.summary_scroll ?? 0)
  ...
  }, [summaryData, resumeState, meta?.lecture?.id])
  ...
  useEffect(() => {
    if (!summaryData?.summary?.content) return
    const sections = extractToc(summaryData.summary.content)
