...
function ContentSkeleton() {
  return (
    <div style={{ padding: '24px 0' }}>
      {[...Array(6)].map((_, i) => (
        <div key={i} style={{ height: i === 0 ? 28 : 16, background: 'linear-gradient(90deg, var(--border) 25%, var(--muted) 50%, var(--border) 75%)', backgroundSize: '200% 100%', borderRadius: 8, marginBottom: 16, width: i % 3 === 2 ? '60%' : '100%', animation: 'mn-shimmer 1.5s infinite' }} />
      ))}
...
export default function LectureHub({
  ...
  const { data: sheetPayload,      isLoading: sheetLoading      } = useQuery({ queryKey: ['tab-content', lecture.id, subject.id, 'sheet'],          queryFn: () => fetchTabContent(lecture.id, subject.id, 'sheet'),          enabled: activeTab === 'sheet',          ...qOpts })
  const { data: summaryPayload,    isLoading: summaryLoading    } = useQuery({ queryKey: ['tab-content', lecture.id, subject.id, 'summary'],        queryFn: () => fetchTabContent(lecture.id, subject.id, 'summary'),        enabled: activeTab === 'summary',        ...qOpts })
  const { data: flashcardsPayload, isLoading: flashcardsLoading } = useQuery({ queryKey: ['tab-content', lecture.id, subject.id, 'flashcards'],     queryFn: () => fetchTabContent(lecture.id, subject.id, 'flashcards'),     enabled: activeTab === 'flashcards',     ...qOpts })
  const { data: quizPayload,       isLoading: quizLoading       } = useQuery({ queryKey: ['tab-content', lecture.id, subject.id, 'quiz'],           queryFn: () => fetchTabContent(lecture.id, subject.id, 'quiz'),           enabled: activeTab === 'quiz',           ...qOpts })
  const { data: pyqPayload,        isLoading: pyqLoading        } = useQuery({ queryKey: ['tab-content', lecture.id, subject.id, 'previous_years'], queryFn: () => fetchTabContent(lecture.id, subject.id, 'previous_years'), enabled: activeTab === 'previous_years', ...qOpts })
  ...
  const sheetImageSlots = useMemo<Record<number, string>>(() =>
    Object.fromEntries(Object.entries(sheetPayload?.imageSlots ?? {}).map(([k, v]) => [Number(k), String(v)])),
  ...
  const summaryImageSlots = useMemo<Record<number, string>>(() =>
    Object.fromEntries(Object.entries(summaryPayload?.imageSlots ?? {}).map(([k, v]) => [Number(k), String(v)])),
  ...
  const { data: progressData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('user_progress').select('content_type, progress_percentage, completed').eq('user_id', user!.id).eq('lecture_id', lecture.id)
      return data ?? []
    },
    enabled: !!user?.id,
    staleTime: 1000 * 60 * 5,
  ...
  const { data: bookmarkData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('bookmarks').select('id').eq('user_id', user!.id).eq('lecture_id', lecture.id).eq('bookmark_type', 'lecture').maybeSingle()
      return data ?? null
    },
    enabled: !!user?.id,
    staleTime: 1000 * 60 * 5,
  ...
  useEffect(() => {
    ...
    if (target <= 0) return
    const ready = activeTab === 'sheet' ? (sheetPayload !== undefined && !sheetLoading) : (summaryPayload !== undefined && !summaryLoading)
    if (!ready) return
    scrollRestoredRef.current = true
    setTimeout(() => { document.getElementById('lecture-content-scroll')?.scrollTo({ top: target, behavior: 'smooth' }) }, 400)
  ...
  useEffect(() => {
    ...
    function onScroll() {
      ...
      for (const s of tocSections) {
        ...
        if (!el) continue
        if (el.getBoundingClientRect().top - scrollEl!.getBoundingClientRect().top <= 140) current = s.id
      }
      setActiveSectionId(current)
      if (current !== prevSectionId.current) {
        prevSectionId.current = current
        document.getElementById(`toc-btn-${current}`)?.scrollIntoView({ block: 'nearest', behavior: 'smooth' })
      }
    }
  ...
  function buildResume(overrides: Partial<ResumeState> = {}): ResumeState {
    return { activeTab, sheetScroll: sheetScrollRef.current, summaryScroll: summaryScrollRef.current, flashcardIndex: currentFlashcardIndex, quizIndex: currentQuizIndex, pyqIndex: currentPyqIndex, ...overrides }
  }
  ...
  function handleTocClick(id: string) {
    ...
    scrollEl.scrollTo({ top: el.getBoundingClientRect().top - scrollEl.getBoundingClientRect().top + scrollEl.scrollTop - 96, behavior: 'smooth' })
  }

  function handleProgressUpdate(pct: number) {
    if (Math.abs(pct - displayPctRef.current) >= 1) { displayPctRef.current = pct; setProgressPercent(pct) }
    if (!user) return
    ...
    progressSaveTimer.current = setTimeout(() => {
      ...
      supabase.from('user_progress').upsert({ user_id: user.id, lecture_id: lecture.id, content_type: activeTab, progress_percentage: pct, completed: pct >= 100, last_accessed_at: new Date().toISOString(), updated_at: new Date().toISOString() }, { onConflict: 'user_id,lecture_id,content_type' })
      saveResume(lecture.id, buildResume({ sheetScroll: sheetScrollRef.current, summaryScroll: summaryScrollRef.current }))
    }, 3000)
  }
  ...
  async function handleMarkComplete() {
    ...
    await supabase.from('user_progress').upsert({ user_id: user.id, lecture_id: lecture.id, content_type: activeTab, progress_percentage: next ? 100 : progressPercent, completed: next, last_accessed_at: new Date().toISOString(), updated_at: new Date().toISOString() }, { onConflict: 'user_id,lecture_id,content_type' })
  }

  async function handleToggleBookmark() {
    if (!user) return
    if (isBookmarked) {
      await supabase.from('bookmarks').delete().eq('user_id', user.id).eq('lecture_id', lecture.id).eq('bookmark_type', 'lecture')
  ...
  return (
    <div className="flex" style={{ height: 'calc(100vh - 72px)', overflow: 'hidden' }}>
      ...
      <div id="lecture-content-scroll" className="flex-1 min-w-0" style={{ overflowY: 'auto', height: 'calc(100vh - 72px)', background: C.bg }}>

        <div className="lg:hidden flex gap-1 px-4 pt-3 pb-2 overflow-x-auto" style={{ flexShrink: 0, background: C.card, borderBottom: `1px solid ${C.border}` }}>
          {allTabs.map((tabId) => {
            const cfg = TAB_CONFIG[tabId]; const isActive = activeTab === tabId
            return (
          ...
          {isCurrentTabLocked ? (
            ...
            <>
              {activeTab === 'sheet' && sheet && (
                <SheetReader content={sheet.content ?? ''} title={lecture.title} onProgressUpdate={handleProgressUpdate} userName={displayName} tocSections={tocSections} imageSlots={sheetImageSlots} />
              )}
              {activeTab === 'summary' && summary && (
                <SheetReader content={summary.content ?? ''} title={lecture.title} isSummary onProgressUpdate={handleProgressUpdate} userName={displayName} tocSections={tocSections} imageSlots={summaryImageSlots} />
              )}
              {activeTab === 'flashcards' && flashcards.length > 0 && (
                <FlashcardsViewer flashcards={flashcards} userName={displayName} initialIndex={currentFlashcardIndex} onIndexChange={handleFlashcardIndexChange} onStatsChange={setFlashcardStats} />
              )}
              {activeTab === 'quiz' && quizQuestions.length > 0 && (
                <QuizViewer questions={quizQuestions} lectureId={lecture.id} userName={displayName} initialIndex={currentQuizIndex} onIndexChange={handleQuizIndexChange} onStatsChange={setQuizStats} />
              )}
              {activeTab === 'previous_years' && previousYearQuestions.length > 0 && (
                <PreviousYearsViewer questions={previousYearQuestions} userName={displayName} initialIndex={currentPyqIndex} onIndexChange={handlePyqIndexChange} onStatsChange={setPyqStats} />
              )}
            </>
          )}
        </div>
      ...
      <aside id="lecture-right-sidebar" className="hidden lg:flex"
        ...
        <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
          <div style={{ padding: sidebarCollapsed ? 8 : '14px 16px 10px' }}>
            {!sidebarCollapsed && <p style={{ margin: '0 0 10px', fontSize: 11, fontWeight: 700, color: C.label, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Content</p>}
                ...
                return (
                  <button key={tabId} onClick={() => handleTabChange(tabId)} title={cfg.label}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: sidebarCollapsed ? 0 : 10 }}>
                      <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, borderRadius: 8, background: isActive ? '#DBEAFE' : C.muted, color: isActive ? C.primary : C.label, flexShrink: 0, transition: 'all 0.15s ease' }}>{cfg.icon}</span>
                      {!sidebarCollapsed && <span style={{ fontSize: 13.5, fontWeight: isActive ? 600 : 500 }}>{cfg.label}</span>}
                    </div>
                    {!sidebarCollapsed && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={isActive ? C.primary : C.border} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>}
                  </button>
                )
              })}
            </div>
        ...
        {!sidebarCollapsed && (activeTab === 'sheet' || activeTab === 'summary') && (
          <LectureContentSearch sheetContent={sheet?.content ?? ''} summaryContent={summary?.content ?? ''} activeTab={activeTab} />
        )}
                  ...
                  {tocSections.filter(s => s.level <= 3).map((section) => {
                    ...
                    return (
                      <button key={section.id} onClick={() => handleTocClick(section.id)}
                        ...
                        <span style={{ minWidth: 26, height: 26, borderRadius: '50%', background: isMain ? C.primary : '#EEF3FF', color: isMain ? '#fff' : C.primary, fontSize: isMain ? 11 : 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, padding: '0 4px' }}>{numLabel}</span>
                        <span style={{ fontSize: isMain ? 13 : 12, fontWeight: isMain ? 700 : 500, color: isMain ? C.ink : C.ink2, lineHeight: 1.4 }}>{section.label}</span>
                      </button>
                    )
                  })}
                </nav>
        ...
        {!sidebarCollapsed && !!user && (
          <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, padding: '14px 16px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <Link href={`/${universityId}/${subjectSlug ?? subject.id}`}
...
function NotesPanel({ lectureId, supabaseClient }: NotesPanelProps) {
  ...
  useEffect(() => {
    ...
    supabaseClient.from('user_notes').select('id, note_content').eq('user_id', user.id).eq('lecture_id', lectureId).maybeSingle()
      .then(({ data }) => { if (data) { setNote(data.note_content ?? ''); setNoteId(data.id) }; setLoading(false) })
  ...
  async function saveNote(content: string) {
    if (!user) return
    if (noteId) {
      await supabaseClient.from('user_notes').update({ note_content: content, updated_at: new Date().toISOString() }).eq('id', noteId)
    } else {
      const { data } = await supabaseClient.from('user_notes').insert({ user_id: user.id, lecture_id: lectureId, note_content: content }).select('id').maybeSingle()
