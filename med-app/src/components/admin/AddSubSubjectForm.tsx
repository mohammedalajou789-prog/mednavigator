...
import { createSubSubjectSchema, type CreateSubSubjectFormValues } from '@/lib/validations/content'

interface Props {
  subjectId: string
}

export default function AddSubSubjectForm({ subjectId }: Props) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CreateSubSubjectFormValues>({
    resolver: zodResolver(createSubSubjectSchema),
    defaultValues: {
      title: '',
      description: '',
    },
  ...
  const onSubmit = async (values: CreateSubSubjectFormValues) => {
    setIsSubmitting(true)
    setServerError(null)

    try {
      const res = await fetch('/api/admin/sub-subjects', {
        method: 'POST',
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
        ...
        <div>
          <label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-1.5">
            Sub-Subject Title <span className="text-red-500">*</span>
          </label>
          <input
            id="title"
            type="text"
            placeholder="e.g. Heart Failure, Valvular Disease, Arrhythmias"
      ...
      <div className="flex items-center justify-between mt-6">
        ...
        >
          {isSubmitting ? 'Creating...' : 'Create Sub-Subject'}
        </button>
      </div>
    </form>
  )
