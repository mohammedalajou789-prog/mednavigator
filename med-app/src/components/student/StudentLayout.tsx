...
function iconStyle(active: boolean): React.CSSProperties {
  return { color: active ? '#6E9BFF' : '#7C89A8', flexShrink: 0 }
}
            ...
            <>
              <button
                ...
                style={{
                  ...
                  padding: '7px 12px', borderRadius: '11px', fontSize: '12px',
                  fontWeight: 500, color: '#7C89A8', cursor: 'pointer',
        ...
        <nav className="lg:hidden flex-shrink-0 bg-white border-t border-slate-200 flex items-center justify-around px-2 py-1">
          {[
            ...
            <Link key={item.href} href={item.href} prefetch={false}
              className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg transition-all text-[10px] font-medium ${isActive(item.href) ? 'text-blue-600' : 'text-slate-400'}`}>
              {item.label}
            </Link>
          ))}
        </nav>
